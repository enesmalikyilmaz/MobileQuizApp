...
router.post("/register", async (req, res) => {
    try {
        ...
        if (exists) return res.status(409).json({ message: "Bu email zaten kayýtlý" });
...
router.post("/login", async (req, res) => {
    try {
        ...
        if (!user) return res.status(401).json({ message: "Email veya þifre hatalý" });

        const ok = await bcrypt.compare(password, user.passwordHash);
        if (!ok) return res.status(401).json({ message: "Email veya þifre hatalý" });
